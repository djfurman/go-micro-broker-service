...
func (app *Config) sendMail(w http.ResponseWriter, mailMessage MailPayload) {
	...
	if response.StatusCode != http.StatusAccepted {
		app.errorJSON(w, errors.New("Error calling mail service"), http.StatusFailedDependency)
	...
	var payload jsonResponse
	payload.Error = false
	payload.Message = "mail message sent"
